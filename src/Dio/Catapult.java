...
public class Catapult {
    ...
    public Catapult (UnitController uc) {
        this.uc = uc;
    }
    ...
    private void rearguardAttack (Location target) {
        ...
        if(rearguard && getDistanceToPoint(target) > 16) {
            moveTo(target);
    ...
    private void rearguardToTarget(Location target) {
        if (isRearguard(target) && getDistanceToPoint(target) > 16) {
            moveTo(target);
        }
    ...
    private UnitInfo obtainEnemy (UnitInfo[]array) {
        ...
        for (int i = 0; i < array.length; i++) {
            tower = array[i].getType() == UnitType.TOWER || array[i].getType() == UnitType.MAGE ? i : tower;
    ...
    private void accumulateForces() {
        ...
        if (getDistanceToPoint(meetPoint) > 8)
            moveTo(meetPoint);
